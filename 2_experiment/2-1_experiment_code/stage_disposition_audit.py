...
and we check for the disposition-section heading ("주문"), which marks the holding.
...
# CURRENT-disposition leakage: a present-tense active sentencing OF THE DEFENDANT,
# or the holding-section heading. (The prior-record section narrates priors in the past tense.)
RE_DISPO = re.compile(r"피고인을[^.]{0,25}(?:징역|금고|벌금|구류)[^.]{0,30}(?:처한다|선고한다)")
RE_JUMUN = re.compile(r"주\s*문")
...
print(f"  facts matching a CURRENT-disposition pattern (leakage candidates): {n_dispo} "
      f"({100*n_dispo/n:.2f}%)")
print(f"  facts containing the holding heading '주문': {int(jumun_hits)}")
